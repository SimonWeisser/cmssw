...
public:
  ...
  void plotDMR(const std::string& plotVar="medianX",Int_t minHits = 50, const std::string& options = "plain", const std::string& xScale = "minmax", const std::string& xScaleValue = "5 5"); // plotVar=mean,meanX,meanY,median,rms etc., comma-separated list can be given; minHits=the minimum hits needed for module to appear in plot; options="plain" for regular DMR, "split" for inwards/outwards split, "layers" for layerwise DMR, "layer=N" for Nth layer, or combination of the previous (e.g. "split layers")
